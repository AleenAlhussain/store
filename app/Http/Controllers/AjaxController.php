...
class AjaxController extends Controller
{
    ...
    public function getData(Request $request)
    {
        if ($request->locale) {
            ...
            if (in_array($request->locale, $code)) {
                $this->lang = $request->locale;
                $this->country = $translations[$request->locale];
            }
        }
        ...
        if ($request->ajax()) {
            ...
            if ($request->key == 'categories') {
                $content = Cache::rememberForever(app()->getLocale().$this->site . $request->key . '-' . $request->val . '-' . $country . '_' . $lang, function () use ($request, $output, $country, $lang) {
                    $gplay = new GPlayApps();
                    ...
                    if ($request->val == 'app') {
                        ...
                        foreach ($categories as $cat) {
                            ...
                            if (strpos($cat['id'], 'GAME') === false) {
                                ...
                                $output .= ' <li><a href="' . route('home.category', [app()->getLocale(),$cat_url]) . '" title="' . $cat['name'] . '"><i class="' . strtolower(str_replace('_', '-', str_replace(['_AND', 'FAMYLY_', 'FAMILY_'], '', $cat['id']))) . '"></i>' . $cat['name'] . '</a></li>';
                            }
                        }
                        ...
                        foreach ($categories as $cat) {
                            ...
                            if (strpos($cat['id'], 'GAME') !== false && $cat['id'] !== 'GAME') {
                                $cat_url = strtolower($cat['id']);
                                $cat_url = str_replace('_','-',$cat_url);
                                $output .= ' <li><a href="' . route('home.category', [app()->getLocale(),$cat_url]) . '" title="' . $cat['name'] . '"><i class="' . strtolower(str_replace('GAME_', '', $cat['id'])) . '"></i>' . $cat['name'] . '</a></li>';
                            }
                        }
                    }
                    return $output;
                ...
                $content = Cache::remember(app()->getLocale() . $this->site . $request->key . '_' . $request->val . '_' . $country . '_' . $lang, $cate_time, function () use ($request, $output, $country, $lang) {
                    $gplay = new GPlayApps();
                    $gplay->setDefaultLocale($lang)
                        ->setDefaultCountry($country);
                            ...
                            foreach ($topGame as $top) {
                                ...
                                $output .= '<li>
                                                <div class="hot_day_img_number">
                                                    ...
                                                    <div class="hot_day_img">
                                                        <a title="' . $top['name'] . ' APK"
                                                           href="' . route('home.detail', [app()->getLocale(), $slug, $top['id']]) . '">
                                                ...
                                                <div class="description">
                                                    <h3>
                                                        <a title="' . $top['name'] . ' APK"
                                                           href="' . route('home.detail', [app()->getLocale(), $slug, $top['id']]) . '">' . $top['name'] . '</a>
                                                    </h3>
                                                    ...
                                                    <div class="down_btn">
                                                        ...
                                                        <a href="' . route('home.detail', [app()->getLocale(), $slug, $top['id']]) . '"
                    ...
                    if ($request->key == 'footer') {
                        if ($request->val == 'game') {
                            ...
                            foreach ($topGame as $top) {
                                ...
                                $slug = Str::slug($top['name'], '-');
                                if (empty($slug)) $slug = str_slug($top['name']);
                                $output .= '<li><a title="' . $top['name'] . ' APK"  href="' . route('home.detail', [app()->getLocale(), $slug, $top['id']]) . '">' . $top['name'] . '</a></li>';
                            }
                        } else {
                        ...
                        foreach ($similarApps as $top) {
                            ...
                            $output .= '<li class="col-sm-4 col-xs-6 product_item" >
                                        ...
                                        <div class="description" >
                                            ...
                                            <div class="down_btn" >
                                                <p> Download ' . $top['name'] . '</p >
                                                <a href ="' . route('home.detail', [app()->getLocale(), $slug, $top['id']]) . '" class="btn btn_down" title = "Download ' . $top['name'] . '" >' . __('web.installapk') . '</a >
                                            </div>
                                        </div >
                                    </li >';
                        }
                    ...
                    if ($request->key == 'developer') {
                        ...
                        foreach ($apps as $top) {
                            ...
                            $output .= '<li class="col-sm-4 col-xs-6 product_item" >
                                        <div class="product_img" >
                                            <a title = "Digital World"
                                               href = "' . route('home.detail', [app()->getLocale(), app()->getLocale(), $slug, $top['id']]) . '" >
                                                <img alt = "Digital World" src = "' . $top['icon'] . '" >
                                            </a >
                                        </div >
                                        <div class="description" >
                                            <h3 >
                                                <a title = "' . $top['name'] . '"
                                                   href="' . route('home.detail', [app()->getLocale(), app()->getLocale(), $slug, $top['id']]) . '" >' . $top['name'] . '</a >
                                            </h3 >
                                            <div class="stars" >
                                                <span title = "' . $top['name'] . ' average rating ' . round($top['score'], 1) . '"
                                                      style = "width:' . (((int)$top['score'] / 5) * 100) . '%;" ></span >
                                            ...
                                            <div class="down_btn" >
                                                <p > Download ' . $top['name'] . '</p >
                                                <a href ="' . route('home.detail', [app()->getLocale(), app()->getLocale(), $slug, $top['id']]) . '" class="btn btn_down" title = "Download ' . $top['name'] . '" >' . __('web.installapk') . '</a >
                                            </div >
                                        </div >
                                    </li >';
                        }
    ...
    public function getSearch(Request $request)
    {
        ...
        if ($request->ajax()) {
            $content = Cache::remember($this->site . $search_cf . $request->q . '_' . $country . '_' . $lang, $cate_time, function () use ($request, $country, $lang, $search_cf) {
                $output = '';
                if ($search_cf == 'play') {
                    $gplay = new GPlayApps();
                    $gplay
                    ...
                    foreach ($search as $top) {
                        ...
                        $output .= '<li class="col-sm-5ths col-xs-6">
                                <div class="product_item">
                                  <div class="product_img">
                                    <a title="' . $top['name'] . '" href="' . route('home.detail', [app()->getLocale(), $slug, $top['id']]) . '">
                                      <img alt="' . $top['name'] . '" src="' . $top['icon'] . '">
                                    </a>
                                  </div>
                                  <div class="description">
                                    <h3>
                                      <a title="' . $top['name'] . '" href="' . route('home.detail', [app()->getLocale(), $slug, $top['id']]) . '">' . $top['name'] . '</a>
                                    </h3>
                                    <div class="down_btn">
                                      <a href="' . route('home.detail', [app()->getLocale(), $slug, $top['id']]) . '" class="btn btn_down" title="Download ' . $top['name'] . '">
                                        ' . __('web.installapk') . '
                                    </a>
                                    </div>
                                  </div>
                    ...
                    foreach ($apps as $top) {
                        ...
                        $output .= '<li class="col-sm-5ths col-xs-6">
                                <div class="product_item">
                                  <div class="product_img">
                                    <a title="' . $top['title'] . '" href="' . route('home.detail', [app()->getLocale(), $top['slug'], $top['appid']]) . '">
                                      <img alt="' . $top['title'] . '" src="' . asset('public/images/') . '/' . $top['cover'] . '">
                                    </a>
                                  </div>
                                  <div class="description">
                                    <h3>
                                      <a title="' . $top['title'] . '" href="' . route('home.detail', [app()->getLocale(), $top['slug'], $top['appid']]) . '">' . $top['title'] . '</a>
                                    </h3>
                                    <div class="down_btn">
                                      <a href="' . route('home.detail', [app()->getLocale(), str_slug($top['title'], '-'), $top['appid']]) . '" class="btn btn_down" title="Download ' . $top['title'] . '">
                                        ' . __('web.installapk') . '
                                    </a>
                                    </div>
                                  </div>
    ...
    public function getApps(Request $request)
    {
        ...
        if ($request->ajax()) {
            $content = Cache::remember($this->site . 'admingetApps_' . $limit . $request->type . $request->q . '_' . $country . '_' . $lang, $cate_time, function () use ($request, $country, $lang, $limit) {
                $output = '';
                $gplay = new GPlayApps();
                $gplay
                    ->setDefaultLocale($lang)
                ...
                if ($request->type == 'cate_top') {
                    ...
                    if (strpos($q, 'GAME') === false) {
                        $search = $gplay->getListApps(
                            $q,
                            null,
                            $limit);
                    }else{
