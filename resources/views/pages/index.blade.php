                ...
                <div class="box data-table-list">
                    ...
                    <div class="box-body no-padding">
                        <div class="basic-tb-hd clearfix">
                            <h2 class="panel-title pull-left" style="padding-top: 7.5px;">Pages<br><span class="bread-ntd" style="font-size: 14px;font-weight: normal"> Click on and drag title to a new spot within the list to sort.</span></h2>
                            <div class="btn-group pull-right">
                                <a href="{{action('PageController@create')}}" class="btn btncus btn_blue"><i
                                        class="notika-icon notika-draft"></i> Add New Page</a>
                            </div>
                        </div>
                        <div class="table-responsive">
                            <table class="table table-striped">
                                ...
                                @foreach($pages as $k=>$row)
                                    <tr id="{{ $row->id }}" class="ui-sortable-handle">
                                        <td>{{$row->id}}</td>
                                        <td>{{$row->title}}</td>
                                        <td style="text-align: right"><a
                                                href="{{action('PageController@edit', $row->id)}}"
                                                class="btn btncus btn_blue"><i class="notika-icon notika-draft"></i>
                                                Edit
                                            </a>
                                            <a href="javascript:void(0);"
                                               data-url="{{action('PageController@destroy', $row->id)}}"
                                               data-id="{{$row->id}}"
                                               class="_delete_data">
            ...
            $(".sortable-posts").sortable({
                stop: function () {
                    ...
                    $.map($(this).find('tr'), function (el) {
                        ...
                        var sort = $(el).index();
                        $.ajax({
                            url: '{{action('PageController@order')}}',
                            type: 'GET',
                            data: {
                                id: id,
                                sort: sort
